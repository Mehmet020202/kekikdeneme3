        ...
        for (domain in alternativeDomains) {
            try {
                ...
                if (response.isSuccessful) {
                    Log.d("FLMMD", "Working domain found: $domain")
                    return domain
                }
            } catch (e: Exception) {
                Log.d("FLMMD", "Domain $domain failed: ${e.message}")
            }
        }
        ...
        try {
            ...
            document.select("div.alternates a").forEach {
                ...
                try {
                    ...
                    Log.d("FLMMD", "Error processing alternate: ${e.message}")
                }
            }

            // 2. Video player iframe'leri ara
            if (!foundLinks) {
                document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null) {
                        Log.d("FLMMD", "Found iframe: $iframeSrc")
            ...
            // 3. Video element'leri ara
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    val videoSrc = fixUrlNull(source.attr("src"))
                    if (videoSrc != null) {
                        Log.d("FLMMD", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("FLMMD", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("FLMMD", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("FLMMD", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}

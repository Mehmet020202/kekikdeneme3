...
class DiziYou : MainAPI() {
    override var mainUrl              = "https://www.diziyou15.com"
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        try {
            ...
            // 1. Orijinal diziyouPlayer yapısı
            ...
            if (!foundLinks) {
                document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null) {
                        Log.d("DZY", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DZY", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DZY", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DZY", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("DZY", "Error in loadLinks: ${e.message}")
            return false
        }
    }

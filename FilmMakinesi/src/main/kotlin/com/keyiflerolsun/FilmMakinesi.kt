        ...
        try {
            ...
            if (!foundLinks) {
                document.select("div.video-player, div.player, div.embed-player, div#player").forEach { player ->
                    ...
                    if (iframe != null) {
                        Log.d("FLMM", "Found player iframe: $iframe")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("FLMM", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("FLMM", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("FLMM", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            if (!foundLinks) {
                try {
                    ...
                    if (videoUrl != null && videoUrl.isNotEmpty()) {
                        Log.d("FLMM", "Found AJAX video URL: $videoUrl")
                        callback.invoke(
                            newExtractorLink(
                                source = "AJAX Video",
                                name = "AJAX Video",
                    ...
                    Log.d("FLMM", "Error with AJAX request: ${e.message}")
                }
            }
            ...
            if (!foundLinks) {
                document.select("iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null && (iframeSrc.contains("player") || iframeSrc.contains("embed"))) {
                        Log.d("FLMM", "Found generic iframe: $iframeSrc")
            ...
            Log.d("FLMM", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}

...
private fun Element.toMainPageResult(): SearchResponse? {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val document = app.get(
            ...
            // 1. Orijinal video-area yapısı
            var iframe = document.selectFirst("div#video-area iframe")?.attr("src")
            if (iframe != null) {
                Log.d("DZBX", "Found video-area iframe: $iframe")
        iframeDecode(data, iframe, subtitleCallback, callback)
            ...
            if (!foundLinks) {
                document.select("div.video-player, div.player, div.embed-player, div#player").forEach { player ->
                    ...
                    if (playerIframe != null) {
                        Log.d("DZBX", "Found player iframe: $playerIframe")
                        iframeDecode(data, playerIframe, subtitleCallback, callback)
                        foundLinks = true
                    }
                }
            }
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DZBX", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DZBX", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DZBX", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            if (!foundLinks) {
                try {
                    ...
                    if (videoUrl != null && videoUrl.isNotEmpty()) {
                        Log.d("DZBX", "Found AJAX video URL: $videoUrl")
                        callback.invoke(
                            newExtractorLink(
                                source = "AJAX Video",
                                name = "AJAX Video",
                    ...
                    Log.d("DZBX", "Error with AJAX request: ${e.message}")
                }
            }
            ...
            if (!foundLinks) {
                document.select("iframe").forEach { iframeElement ->
                    val iframeSrc = fixUrlNull(iframeElement.attr("src")) ?: fixUrlNull(iframeElement.attr("data-src"))
                    if (iframeSrc != null && (iframeSrc.contains("player") || iframeSrc.contains("embed"))) {
                        Log.d("DZBX", "Found generic iframe: $iframeSrc")
                        iframeDecode(data, iframeSrc, subtitleCallback, callback)
                        foundLinks = true
                    }
                }
            }

            // 7. Video toolbar alternatifleri
            if (foundLinks) {
        document.select("div.video-toolbar option[value]").forEach {
            ...
            val subDoc  = app.get(
                ...
                interceptor = interceptor
                    ...
                    iframe = subDoc.selectFirst("div#video-area iframe")?.attr("src")
                    if (iframe != null) {
                        Log.d("DZBX", "Found toolbar iframe: $iframe")
            iframeDecode(data, iframe, subtitleCallback, callback)
                    }
                }
            }
            ...
            Log.d("DZBX", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}
